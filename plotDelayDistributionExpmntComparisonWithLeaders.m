...
for permCtr = 1:2
    ...
    for selfAlign = [false, true]
        ...
        for alphaCtr = 1:numAlphas
            ...
            for betaCtr = 1:numBetas
                ...
                for repCtr = 1:numRepeats
                    ...
                    startSample=1;%randi(T-burnIn-nTimePoints,1);
                    ...
                    if order(repCtr) >= minOrder && order(repCtr) <= maxOrder
                        ...
                        order(repCtr) = NaN; % don't keep order values in the wrong range
                    end
                end
                if plotCurrent
                    %                     plot(binCentres,squeeze(histPeakLag(alphaCtr,betaCtr,:,2:end-1)),...
                    %                         'Color',[0 0 1 1/sqrt(numRepeats)])
                    ...
                    filename = ['manuscript/figures/diagnostics/delayDist_T' num2str(T) '_N' num2str(N) ...
                        '_Nl' num2str(Nl) '_L' num2str(L,'%2.0e\n') '_a' num2str(alpha,precision) ...
                        '_b' num2str(beta,precision) '_selfAlign' num2str(selfAlign)
                        ];
        ...
        disp(['Best fit for ' experimentalReference{permCtr} ' of ' num2str(bestFit) ...
            ...
            ' (self-alignment ' num2str(selfAlign) ')'])
        %% save results
        save(['delayCorrResults_N' num2str(N) '_Nl' num2str(Nl) '_L' num2str(L) '_selfAlign' num2str(selfAlign) ...
